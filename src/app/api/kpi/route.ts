...
import {
  kpiTargets, kpiManualInputs,
  beneficiaries, trainings, techTransfers,
  productServices, transactions, valueChains,
...
export async function GET(req: NextRequest) {
  ...
  try {
    const targets = await db.select().from(kpiTargets).where(eq(kpiTargets.isActive, true)).orderBy(kpiTargets.sortOrder)
    ...
    const [
      kpi01, kpi02, kpi03, kpi04, kpi05,
      kpi06, kpi07, kpi08, kpi09, kpi10, kpi11,
    ] = await Promise.all([
      // KPI01 — SE partners (area + date filter on created_at)
      db.select({ count: count() }).from(partners).where(
        and(eq(partners.partnerType, "SE"), eq(partners.isActive, true),
          ...filterConds([areaEq(partners.area)]))),

      // KPI02 — trained entrepreneurs/health_personnel (area + date on training_date)
      db.select({ count: countDistinct(trainings.participantCode) }).from(trainings)
        .where(and(
      ...
      db.select({ count: countDistinct(techTransfers.participantCode) }).from(techTransfers)
        .where(and(
          ...filterConds([
      ...
      db.select({ count: count() }).from(productServices).where(
      ...
      db.select({ count: count() }).from(innovations)
        .where(and(eq(innovations.innovationType, "technology"), ...filterConds([areaEq(innovations.area)]))),

      // KPI10 — business_system innovations (area filter)
      db.select({ count: count() }).from(innovations)
        .where(and(eq(innovations.innovationType, "business_system"), ...filterConds([areaEq(innovations.area)]))),

      // KPI11 — published manuals (no area — knowledge is global)
      db.select({ count: count() }).from(knowledgeContents)
        .where(and(eq(knowledgeContents.contentType, "manual"), eq(knowledgeContents.isPublished, true))),
    ])

    const vcRow = kpi05[0]
    const vcPercent = vcRow.grandTotal > 0
