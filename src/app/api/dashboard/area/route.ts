...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [beneficiaryByArea, transactionByArea, trainingByArea, employmentByArea, partnerByArea] =
      await Promise.all([
        db
          .select({ area: beneficiaries.area, count: count() })
          .from(beneficiaries)
          .where(area ? eq(beneficiaries.area, area) : undefined)
          .groupBy(beneficiaries.area),

        db
          .select({ area: transactions.area, count: count(), total: sum(transactions.amount) })
          .from(transactions)
          .where(area ? eq(transactions.area, area) : undefined)
          .groupBy(transactions.area),

        db
          .select({ area: trainings.area, count: count() })
          .from(trainings)
          .where(area ? eq(trainings.area, area) : undefined)
          .groupBy(trainings.area),

        db
          .select({ area: employments.area, count: countDistinct(employments.personCode) })
          .from(employments)
          .where(
            area
        ...
        db
          .select({ area: partners.area, count: count() })
          .from(partners)
          .where(
            area
