...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [revenue, byStatus, byProduct, valueChainSummary, byPaymentMethod] = await Promise.all([
      db
        .select({
      ...
      db
        .select({ status: transactions.status, count: count(), total: sum(transactions.amount) })
        .from(transactions)
      ...
      db
        .select({ method: transactions.paymentMethod, count: count() })
        .from(transactions)
        .where(conditions.length ? and(...conditions) : undefined)
    ...
    const vcPercent =
      totalAmt > 0 ? Number(((Number(vc.communityTotal) / totalAmt) * 100).toFixed(2)) : 0
